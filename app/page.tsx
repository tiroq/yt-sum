...
type QueueHealth = { total: number; queued: number; processing: number; completed: number; failed: number; cancelled: number; current_stage: string | null; current_video_id: string | null; current_progress: number | null };
type PipelineNode = { id: string; count: number; queued: number; processing: number; failed: number; completed: number; video_ids: string[] };
type Health = { status: string; queue_paused: boolean; library: string; queues?: { download: QueueHealth; llm: QueueHealth }; pipeline?: PipelineNode[]; components: Record<string, { ready: boolean; version?: string; engine?: string; address?: string; state?: string; reason?: string }> };
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {!online ? <OfflineState message={error || t.offline} onRetry={refresh} language={language} /> : view === "settings" && settings ? (
          ...
          <div className="status-page status-layout"><SystemStatus health={health} settings={settings} language={language} onRescan={async () => { await request("/library/rescan", { method: "POST" }); await refresh(); }} /><SourceUpdatePanel language={language} /></div>
        ) : detail ? (
          <>
...
function PipelineGraph({ pipeline, language }: { pipeline?: PipelineNode[]; language: "ru" | "en" }) {
  if (!pipeline?.length) return null;
  const labels: Record<string, string> = language === "ru" ? { queued: "Очередь", metadata: "Metadata", thumbnail: "Превью", "transcript-selection": "Выбор текста", "subtitle-download": "Субтитры", "audio-download": "Аудио", transcribing: "ASR", "transcript-ready": "Текст готов", summarizing: "Summary", "summary-map": "Map", "summary-reduce": "Reduce", "summary-final": "Финальный summary", "running-prompt": "Промпт", "speech-synthesis": "Озвучивание", "saving-audio": "Сохранение аудио", complete: "Готово", attention: "Ошибка", cancelled: "Остановлено" } : { queued: "Queued", metadata: "Metadata", thumbnail: "Preview", "transcript-selection": "Text selection", "subtitle-download": "Subtitles", "audio-download": "Audio", transcribing: "ASR", "transcript-ready": "Transcript ready", summarizing: "Summary", "summary-map": "Map", "summary-reduce": "Reduce", "summary-final": "Final summary", "running-prompt": "Prompt", "speech-synthesis": "Speech", "saving-audio": "Saving audio", complete: "Complete", attention: "Failed", cancelled: "Cancelled" };
  const terminalStates = new Set(["complete", "attention", "cancelled"]);
  const nodes = pipeline.filter((node) => !terminalStates.has(node.id) && (node.count || ["queued", "metadata", "transcript-ready", "summarizing"].includes(node.id)));
  return <section className="pipeline-panel"><div className="section-heading"><div><span className="overline">PIPELINE</span><h2>{language === "ru" ? "Граф обработки" : "Processing pipeline"}</h2></div><span className="queue-global-state">{language === "ru" ? "автообновление" : "auto-updated"}</span></div><div className="pipeline-graph">{nodes.map((node, index) => <div className="pipeline-step" key={node.id}><article className={`pipeline-node ${node.processing ? "active" : ""} ${node.failed ? "failed" : ""}`} title={node.video_ids.join(", ")}><div className="pipeline-node-top"><strong>{labels[node.id] ?? node.id}</strong><b>{node.count}</b></div><div className="pipeline-node-stats"><span>{node.processing} {language === "ru" ? "активн." : "active"}</span><span>{node.queued} {language === "ru" ? "ждут" : "queued"}</span><span>{node.failed} {language === "ru" ? "ошибок" : "failed"}</span></div>{node.video_ids.length ? <small>{node.video_ids.slice(0, 3).join(" · ")}{node.video_ids.length > 3 ? " …" : ""}</small> : null}</article>{index < nodes.length - 1 ? <span className="pipeline-arrow" aria-hidden="true">→</span> : null}</div>)}</div></section>;
}
...
function SystemStatus({ health, settings, language, onRescan }: { health: Health | null; settings: Settings | null; language: "ru" | "en"; onRescan: () => Promise<void> }) {
  ...
  const queueLabels = language === "ru" ? { download: "Загрузка и транскрипция", llm: "Языковые модели" } : { download: "Download and transcript", llm: "Language models" };
  const queueDiagnostics = Object.entries(health.queues ?? {}).map(([key, queue]) => ({ key, queue, label: queueLabels[key as "download" | "llm"] ?? key }));
  const queuePanel = queueDiagnostics.length ? <section className="queue-diagnostics"><div className="section-heading"><div><span className="overline">QUEUES</span><h2>{language === "ru" ? "Состояние очередей" : "Queue status"}</h2></div><span className="queue-global-state">{health.queue_paused ? (language === "ru" ? "На паузе" : "Paused") : (language === "ru" ? "Работают" : "Running")}</span></div><div className="queue-diagnostics-grid">{queueDiagnostics.map(({ key, queue, label }) => <article className="queue-diagnostic-card" key={key}><div className="queue-diagnostic-title"><h3>{label}</h3><span>{queue.processing ? (language === "ru" ? "В работе" : "Active") : (language === "ru" ? "Ожидание" : "Idle")}</span></div><div className="queue-stat-grid"><div><strong>{queue.queued}</strong><small>{language === "ru" ? "в очереди" : "queued"}</small></div><div><strong>{queue.processing}</strong><small>{language === "ru" ? "в работе" : "processing"}</small></div><div><strong>{queue.completed}</strong><small>{language === "ru" ? "готово" : "complete"}</small></div><div><strong>{queue.failed}</strong><small>{language === "ru" ? "ошибки" : "failed"}</small></div><div><strong>{queue.cancelled}</strong><small>{language === "ru" ? "остановлено" : "cancelled"}</small></div></div>{queue.current_stage ? <p className="queue-current-stage">{queue.current_stage}{queue.current_progress !== null ? ` · ${Math.round(queue.current_progress * 100)}%` : ""}{queue.current_video_id ? ` · ${queue.current_video_id}` : ""}</p> : <p className="queue-current-stage">{language === "ru" ? "Нет активной задачи" : "No active task"}</p>}</article>)}</div></section> : null;
