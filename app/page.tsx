...
function QueuePanel({ jobs, videos, paused, close, onSelectVideo, refresh, language }: { jobs: Job[]; videos: VideoItem[]; paused: boolean; close: () => void; onSelectVideo: (videoId: string) => void; refresh: () => Promise<void>; language: "ru" | "en" }) {
  ...
  async function clearInactiveJobs() {
    if (!inactiveCount || !window.confirm(language === "ru" ? `Удалить все неактивные записи (${inactiveCount})? Активные задачи останутся в очереди.` : `Remove all inactive processing records (${inactiveCount})? Active tasks will remain queued.`)) return;
  ...
  return <div className="queue-backdrop" onClick={close}><aside className="queue-panel" role="dialog" aria-modal="true" aria-label={language === "ru" ? "Очереди обработки" : "Processing queues"} onClick={(event) => event.stopPropagation()}><div className="queue-heading"><div><span className="overline">BACKGROUND</span><h2>{language === "ru" ? "Очереди обработки" : "Processing queues"}</h2></div><div className="queue-heading-actions"><button className="icon-button queue-clear-inactive" onClick={() => void clearInactiveJobs()} disabled={!inactiveCount} aria-label={language === "ru" ? "Очистить неактивные записи" : "Clear inactive entries"} title={language === "ru" ? "Удалить неактивные записи обработки" : "Remove inactive processing records"}><Trash2 size={15} /></button><button className="icon-button" onClick={close}><X size={18} /></button></div></div><div className="queue-controls"><button className="queue-control" onClick={() => void queueAction(paused ? "/jobs/resume" : "/jobs/pause")}>{paused ? <Play size={16} /> : <Pause size={16} />}{paused ? (language === "ru" ? "Продолжить" : "Resume") : (language === "ru" ? "Пауза" : "Pause")}</button><button className="queue-stop-all" onClick={() => { if (window.confirm(language === "ru" ? "Остановить все активные и ожидающие задачи?" : "Stop all active and queued jobs?")) void queueAction("/jobs/stop"); }}>{language === "ru" ? "Остановить всё" : "Stop all"}</button></div>{renderLane(downloadJobs, language === "ru" ? "Получение данных" : "Data acquisition")}{renderLane(llmJobs, language === "ru" ? "Языковые модели" : "Language models")}{renderLane(ttsJobs, language === "ru" ? "Озвучивание" : "Text to speech")}</aside></div>;
}
