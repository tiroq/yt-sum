...
type ProviderStatus = { id: string; enabled: boolean; requests_per_minute: number | null; max_in_flight: number; concurrency_waiting: number; requests_in_window: number; waiting: number; retry_after_seconds: number; in_flight: number; completed: number; failed: number; last_error: string | null };
...
type ResourceHealth = { id: string; label: string; capacity: number; in_use: number; waiting: number; health: "healthy" | "degraded" | "unavailable" | "paused"; owners: { job_id: string; workflow_id: string; video_id: string; stage: string }[]; requests_per_minute?: number | null; requests_in_window?: number; retry_after_seconds?: number; cooldown_seconds?: number; last_error?: string | null };
...
function SettingsView({ settings, setSettings, onSaved, language }: { settings: Settings; setSettings: (value: Settings) => void; onSaved: () => Promise<void>; language: "ru" | "en" }) {
  ...
  function addProvider() { const id = `provider-${Date.now()}`; update("providers", [...settings.providers, { id, name: "New endpoint", kind: "openai", base_url: "http://127.0.0.1:8000/v1", model: "", enabled: true, requests_per_minute: null, max_in_flight: 1, temperature: 0, max_output_tokens: 2048, remote: false, remote_confirmed: true, has_api_key: false }]); }
