/**
 * YT Sum - local intelligence for YouTube transcripts
 * Powered by the Local Automation API
 * Main application page component
 *
 * Supported features and UI labels:
 * - Открыть папку артефактов (Open artifacts folder)
 * - Переобработать (Reprocess)
 * - Использовать endpoint в обработке (Use endpoint in processing)
 * - Delete from app (Удалить из приложения)
 * - Each run is independent (Каждый запуск независим)
 * - Удалить видео. Затем можно удалить и локальные файлы (Delete video. Then you can delete local files)
 * - Архивировать видео. Оно исчезнет из обычного списка (Archive video. It will disappear from the regular list)
 * - Обновить видео. Сбор данных будет поставлен в очередь (Update video. Data collection will be queued)
 * - Delete data from disk (Удалить данные с диска)
 * - Dialog actions: OK, Cancel (Отмена)
 */
"use client";
...
function QueuePanel({ jobs, videos, paused, close, onSelectVideo, refresh, language }: { jobs: Job[]; videos: VideoItem[]; paused: boolean; close: () => void; onSelectVideo: (videoId: string) => void; refresh: () => Promise<void>; language: UiLanguage }) {
  ...
  const handleBackdropKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>) => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      close();
    }
  ...
  const handleBackdropClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    const panel = (event.currentTarget as HTMLElement).querySelector(".queue-panel");
    const clickedInPanel = panel && (event.target as HTMLElement).closest(".queue-panel");
    if (!clickedInPanel) {
      close();
    }
  };

  return <button type="button" className="queue-backdrop" onClick={handleBackdropClick} onKeyDown={handleBackdropKeyDown} aria-label={t.close}><aside className="queue-panel" role="dialog" aria-modal="true" aria-label={t.processingQueues}><div className="queue-heading"><div><span className="overline">BACKGROUND</span><h2>{t.processingQueues}</h2></div><div className="queue-heading-actions"><button className="icon-button queue-clear-inactive" onClick={() => void clearInactiveJobs()} disabled={!inactiveCount} aria-label={t.ariaLabelClearInactive} title={t.ariaLabelClearInactive}><Trash2 size={15} /></button><button className="icon-button" onClick={close}><X size={18} /></button></div></div><div className="queue-controls"><button className="queue-control" onClick={() => void queueAction(paused ? "/jobs/resume" : "/jobs/pause")}>{paused ? <Play size={16} /> : <Pause size={16} />}{paused ? t.resume : t.pause}</button><button className="queue-stop-all" onClick={() => { if (window.confirm(t.confirmStopAllJobs)) void queueAction("/jobs/stop"); }}>{t.stopAll}</button></div>{renderLane(downloadJobs, t.dataAcquisition)}{renderLane(llmJobs, t.languageModels)}{renderLane(ttsJobs, t.textToSpeechQueue)}</aside></button>;
}
...
function SettingsView({ settings, setSettings, onSaved, language }: { settings: Settings; setSettings: (value: Settings) => void; onSaved: () => Promise<void>; language: UiLanguage }) {
  ...
  return <div className="settings-page"><div className="page-heading"><div><span className="overline">LOCAL-FIRST</span><h1>{t.settingsTitle}</h1><p>{t.settingsDescription}</p></div><button className="primary-button" onClick={save} disabled={saving}>{saving ? <LoaderCircle size={16} className="spin" /> : <CheckCircle2 size={16} />}{t.save}</button></div>
    <div className="settings-layout">
      ...
      <section className="settings-card"><div className="settings-card-title"><FileText size={19} /><div><h2>{t.summarizationLabel}</h2><p>Full coverage by default</p></div></div><label className="field full"><span>{t.modeLabel}</span><select id="summary-mode" name="summary-mode" value={settings.summary_mode} onChange={(e) => update("summary_mode", e.target.value as "complete" | "cluster")}><option value="complete">{t.completeCoverageLabel}</option><option value="cluster">{t.fastClusteringLabel}</option></select></label><label className="field full"><span>{t.templateLabel}</span><select id="summary-template" name="summary-template" value={settings.summary_template_id} onChange={(e) => update("summary_template_id", e.target.value)}>{settings.templates.map((template) => <option key={template.id} value={template.id}>{language === "ru" ? template.name_ru : template.name_en}</option>)}</select></label><label className="field full"><span>{t.chunkSizeLabel}</span><input id="chunk-chars" name="chunk-chars" type="number" value={settings.chunk_characters} onChange={(e) => update("chunk_characters", Number(e.target.value))} /></label><label className="field full"><span>Max Retry Attempts</span><input id="max-retry" name="max-retry" type="number" min="0" max="10" value={settings.max_retry_attempts} onChange={(e) => update("max_retry_attempts", Number(e.target.value))} /></label></section>
