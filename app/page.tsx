  ...
  return (
    <main className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? "sidebar-open" : ""}`}>
        ...
        <div className="search-box"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t.search} /></div>
...
function AddDialog({ links, setLinks, onClose, onAdd, language }: { links: string; setLinks: (value: string) => void; onClose: () => void; onAdd: () => void; language: "ru" | "en" }) {
  return <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="add-dialog-title"><div className="modal-heading"><div><span className="overline">YOUTUBE</span><h2 id="add-dialog-title">{language === "ru" ? "Добавить в библиотеку" : "Add to library"}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close"><X size={18} /></button></div><p>{language === "ru" ? "Вставьте одну или несколько ссылок — по одной на строку." : "Paste one or more links, one per line."}</p><textarea value={links} onChange={(event) => setLinks(event.target.value)} placeholder="https://www.youtube.com/watch?v=…" rows={7} aria-label={language === "ru" ? "Ссылки YouTube" : "YouTube links"} /><div className="modal-note"><Clock3 size={16} /><span>{language === "ru" ? "Видео обрабатываются по одному с паузами 30–90 секунд." : "Videos are processed one at a time with 30–90 second pauses."}</span></div><div className="modal-actions"><button className="ghost-button" onClick={onClose}>{language === "ru" ? "Отмена" : "Cancel"}</button><button className="primary-button" onClick={onAdd} disabled={!links.trim()}><Plus size={17} />{language === "ru" ? "Добавить в очередь" : "Add to queue"}</button></div></section></div>;
}
...
function SettingsView({ settings, setSettings, onSaved, language }: { settings: Settings; setSettings: (value: Settings) => void; onSaved: () => Promise<void>; language: "ru" | "en" }) {
  ...
  return <div className="settings-page"><div className="page-heading"><div><span className="overline">LOCAL-FIRST</span><h1>{language === "ru" ? "Настройки" : "Settings"}</h1><p>{language === "ru" ? "Всё хранится на этом Mac. Секреты — только в Связке ключей." : "Everything stays on this Mac. Secrets are stored only in Keychain."}</p></div><button className="primary-button" onClick={save} disabled={saving}>{saving ? <LoaderCircle size={16} className="spin" /> : <CheckCircle2 size={16} />}{language === "ru" ? "Сохранить" : "Save"}</button></div>
    <div className="settings-layout">
      <section className="settings-card"><div className="settings-card-title"><FolderOpen size={19} /><div><h2>{language === "ru" ? "Библиотека и языки" : "Library and languages"}</h2><p>Markdown + JSON</p></div></div><label className="field full"><span>{language === "ru" ? "Папка библиотеки" : "Library folder"}</span><input value={settings.library_dir} onChange={(e) => update("library_dir", e.target.value)} /></label><div className="field-grid"><label className="field"><span>{language === "ru" ? "Основной текст" : "Primary transcript"}</span><input value={settings.primary_language} onChange={(e) => update("primary_language", e.target.value)} /></label><label className="field"><span>{language === "ru" ? "Запасной текст" : "Fallback transcript"}</span><input value={settings.secondary_language} onChange={(e) => update("secondary_language", e.target.value)} /></label><label className="field"><span>{language === "ru" ? "Язык summary" : "Summary language"}</span><input value={settings.summary_language} onChange={(e) => update("summary_language", e.target.value)} /></label><label className="field"><span>{language === "ru" ? "Интерфейс" : "Interface"}</span><select value={settings.interface_language} onChange={(e) => update("interface_language", e.target.value as "ru" | "en")}><option value="ru">Русский</option><option value="en">English</option></select></label></div></section>

      <section className="settings-card"><div className="settings-card-title"><Clock3 size={19} /><div><h2>{language === "ru" ? "Бережная загрузка" : "Respectful downloading"}</h2><p>yt-dlp</p></div></div><div className="field-grid"><label className="field"><span>{language === "ru" ? "Минимальная пауза, сек" : "Minimum delay, sec"}</span><input type="number" value={settings.min_download_delay_seconds} onChange={(e) => update("min_download_delay_seconds", Number(e.target.value))} /></label><label className="field"><span>{language === "ru" ? "Максимальная пауза, сек" : "Maximum delay, sec"}</span><input type="number" value={settings.max_download_delay_seconds} onChange={(e) => update("max_download_delay_seconds", Number(e.target.value))} /></label></div><label className="field full"><span>cookies.txt</span><input value={settings.cookie_file} onChange={(e) => update("cookie_file", e.target.value)} placeholder="~/Downloads/cookies.txt" /></label><label className="field full"><span>{language === "ru" ? "Cookies браузера" : "Browser cookies"}</span><select value={settings.cookie_browser} onChange={(e) => update("cookie_browser", e.target.value)}><option value="">{language === "ru" ? "Не использовать" : "Disabled"}</option><option value="chrome">Chrome</option><option value="safari">Safari</option><option value="firefox">Firefox</option></select></label></section>
      ...
      <section className="settings-card"><div className="settings-card-title"><FileText size={19} /><div><h2>{language === "ru" ? "Суммаризация" : "Summarization"}</h2><p>Full coverage by default</p></div></div><label className="field full"><span>{language === "ru" ? "Режим" : "Mode"}</span><select value={settings.summary_mode} onChange={(e) => update("summary_mode", e.target.value as "complete" | "cluster")}><option value="complete">{language === "ru" ? "Полное покрытие (map-reduce)" : "Complete coverage (map-reduce)"}</option><option value="cluster">{language === "ru" ? "Быстрый кластерный (lossy)" : "Fast clustering (lossy)"}</option></select></label><label className="field full"><span>{language === "ru" ? "Шаблон" : "Template"}</span><select value={settings.summary_template_id} onChange={(e) => update("summary_template_id", e.target.value)}>{settings.templates.map((template) => <option key={template.id} value={template.id}>{language === "ru" ? template.name_ru : template.name_en}</option>)}</select></label><label className="field full"><span>{language === "ru" ? "Размер фрагмента, символов" : "Chunk size, characters"}</span><input type="number" value={settings.chunk_characters} onChange={(e) => update("chunk_characters", Number(e.target.value))} /></label></section>

      <section className="settings-card"><div className="settings-card-title"><Languages size={19} /><div><h2>{language === "ru" ? "Распознавание аудио" : "Audio transcription"}</h2><p>Meeting Transcriber · CoreML</p></div></div><label className="field full"><span>{language === "ru" ? "Движок" : "Engine"}</span><select value={settings.asr_engine} onChange={(e) => update("asr_engine", e.target.value as "whisperkit" | "parakeet")}><option value="whisperkit">WhisperKit</option><option value="parakeet">Parakeet TDT v3</option></select></label><label className="field full"><span>Automation API</span><input value={settings.meeting_transcriber_url} onChange={(e) => update("meeting_transcriber_url", e.target.value)} /></label><label className="check-row"><input type="checkbox" checked={settings.diarization_enabled} onChange={(e) => update("diarization_enabled", e.target.checked)} />{language === "ru" ? "Разделять спикеров" : "Speaker diarization"}</label><label className="check-row"><input type="checkbox" checked={settings.keep_audio} onChange={(e) => update("keep_audio", e.target.checked)} />{language === "ru" ? "Сохранять аудио" : "Keep audio files"}</label></section>
