...
type ProviderStatus = { id: string; enabled: boolean; requests_per_minute: number | null; tokens_per_minute: number; max_in_flight: number; concurrency_waiting: number; requests_in_window: number; tokens_in_window: number; waiting: number; token_waiting: number; retry_after_seconds: number; request_interval_seconds: number; token_retry_after_seconds: number; in_flight: number; completed: number; failed: number; last_error: string | null };
...
type ResourceHealth = { id: string; label: string; capacity: number; in_use: number; waiting: number; health: "healthy" | "degraded" | "unavailable" | "paused"; owners: { job_id: string; workflow_id: string; video_id: string; stage: string }[]; requests_per_minute?: number | null; requests_in_window?: number; request_interval_seconds?: number; tokens_per_minute?: number; tokens_in_window?: number; retry_after_seconds?: number; token_retry_after_seconds?: number; cooldown_seconds?: number; last_error?: string | null };
...
function ProviderUsageStatus({ status, language }: { status?: ProviderStatus; language: "ru" | "en" }) {
  ...
  const usage = [
    `${status.requests_in_window}${status.requests_per_minute ? ` / ${status.requests_per_minute} RPM` : ""}`,
...
function SettingsView({ settings, setSettings, onSaved, language }: { settings: Settings; setSettings: (value: Settings) => void; onSaved: () => Promise<void>; language: "ru" | "en" }) {
  ...
  function addProvider() { const id = `provider-${Date.now()}`; update("providers", [...settings.providers, { id, name: "New endpoint", kind: "openai", base_url: "http://127.0.0.1:8000/v1", model: "", enabled: true, requests_per_minute: null, tokens_per_minute: 0, max_in_flight: 1, temperature: 0, max_output_tokens: 2048, remote: false, remote_confirmed: true, has_api_key: false }]); }
...
function ResourcePanel({ resources, language }: { resources?: ResourceHealth[]; language: "ru" | "en" }) {
  ...
  return <section className="resource-panel"><div className="section-heading"><div><span className="overline">RESOURCES</span><h2>{language === "ru" ? "Ресурсы и ограничения" : "Resources and limits"}</h2></div></div><div className="resource-grid">{resources.map((resource) => <article className={`resource-card ${resource.health}`} key={resource.id}><div className="resource-title"><div><span className={`resource-health ${resource.health}`} /><strong>{resource.label}</strong></div><b>{resource.in_use}/{resource.capacity}</b></div><div className="resource-meter"><span style={{ width: `${Math.min(100, resource.in_use / Math.max(1, resource.capacity) * 100)}%` }} /></div><div className="resource-stats"><span>{resource.waiting} {language === "ru" ? "ожидает" : "waiting"}</span>{resource.requests_per_minute ? <span>{resource.requests_in_window ?? 0}/{resource.requests_per_minute} RPM</span> : null}{resource.request_interval_seconds ? <span>{language === "ru" ? "интервал" : "interval"}: {resource.request_interval_seconds}s</span> : null}{resource.tokens_per_minute ? <span>{resource.tokens_in_window ?? 0}/{resource.tokens_per_minute} TPM</span> : null}{resource.retry_after_seconds ? <span>{language === "ru" ? "RPM лимит" : "RPM limit"}: {resource.retry_after_seconds}s</span> : null}{resource.token_retry_after_seconds ? <span>{language === "ru" ? "TPM лимит" : "TPM limit"}: {resource.token_retry_after_seconds}s</span> : null}{resource.cooldown_seconds ? <span>{language === "ru" ? "Отключён" : "Cooldown"}: {resource.cooldown_seconds}s</span> : null}</div>{resource.owners[0] ? <p>{resource.owners[0].stage}{resource.owners[0].video_id ? ` · ${resource.owners[0].video_id}` : ""}</p> : <p>{language === "ru" ? "Свободен" : "Idle"}</p>}{resource.last_error ? <small>{resource.last_error}</small> : null}</article>)}</div></section>;
}
