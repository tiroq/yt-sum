...
function ResourcePanel({ resources, language }: { resources?: ResourceHealth[]; language: "ru" | "en" }) {
  ...
  return <section className="resource-panel"><div className="section-heading"><div><span className="overline">RESOURCES</span><h2>{language === "ru" ? "Ресурсы и ограничения" : "Resources and limits"}</h2></div></div><div className="resource-grid">{resources.map((resource) => <article className={`resource-card ${resource.health}`} key={resource.id}><div className="resource-title"><div><span className={`resource-health ${resource.health}`} /><strong>{resource.label}</strong></div><b>{resource.in_use}/{resource.capacity}</b></div><div className="resource-meter"><span style={{ width: `${Math.min(100, resource.in_use / Math.max(1, resource.capacity) * 100)}%` }} /></div><div className="resource-stats"><span>{resource.waiting} {language === "ru" ? "ожидает" : "waiting"}</span>{resource.requests_per_minute ? <span>{resource.requests_in_window ?? 0}/{resource.requests_per_minute} RPM</span> : null}{resource.request_interval_seconds ? <span>{language === "ru" ? "интервал" : "interval"}: {resource.request_interval_seconds}s</span> : null}{resource.tokens_per_minute ? <span>{resource.tokens_in_window ?? 0}/{resource.tokens_per_minute} TPM</span> : null}{resource.retry_after_seconds ? <span>{language === "ru" ? "RPM лимит" : "RPM limit"}: {formatDuration(resource.retry_after_seconds)}</span> : null}{resource.token_retry_after_seconds ? <span>{language === "ru" ? "TPM лимит" : "TPM limit"}: {formatDuration(resource.token_retry_after_seconds)}</span> : null}{resource.cooldown_seconds ? <span>{language === "ru" ? "Отключён" : "Cooldown"}: {formatDuration(resource.cooldown_seconds)}</span> : null}</div>{resource.owners[0] ? <p>{resource.owners[0].stage}{resource.owners[0].video_id ? ` · ${resource.owners[0].video_id}` : ""}</p> : <p>{language === "ru" ? "Свободен" : "Idle"}</p>}{resource.last_error ? <small>{resource.last_error}</small> : null}</article>)}</div></section>;
}
